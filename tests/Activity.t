...
subtest "Module Association" => sub{
	...
	is($module->get_schedule()->get_activity_numbers(),1,"Module contains correct number of activities");
	ok($module->get_schedule()->exists_activity($lecture),"Module contains reference to activity");
};
subtest "Student Association" => sub{
	...
	ok($student->get_schedule()->exists_activity($lecture), "Student Person contains a reference to activity");

	is($lecture->get_student_numbers(),1,"Received correct number of students after student add");
	is($student->get_schedule()->get_activity_numbers(),1,"Received correct number of activities in student after add activity to student");
	...
	is($student->get_schedule()->get_activity_numbers(),0,"Received correct number of activities in student after remove activity from student");

	ok(!$lecture->exists_staff($student), "Student remove works");
	ok(!$student->get_schedule()->exists_activity($lecture), "Student no longer contains a reference to activity");
};
subtest "Staff Association" => sub{
	...
	ok($lecturer->get_schedule()->exists_activity($lecture), "Staff Person contains a reference to activity");
	is($lecture->get_staff_numbers(),1,"Received correct number of staff after staff add");
	is($lecturer->get_schedule()->get_activity_numbers(),1,"Received correct number of activities in staff after add activity to staff");

	$lecture->remove_staff($lecturer);
	is($lecturer->get_schedule()->get_activity_numbers(),0,"Received correct number of activities in staff after remove activity from staff");
	...
	ok(!$lecturer->get_schedule()->exists_activity($lecture), "Staff no longer contains a reference to activity");
};
subtest "Room Association" => sub{
	...
	ok($room->get_schedule()->exists_activity($lecture),"First room contains reference to activity");
	
	$lecture->set_room($second_room);
	ok($lecture->get_room()->equals($second_room), "Activity now contains reference to second room");
	ok(!$room->get_schedule()->exists_activity($lecture),"First room no longer contains reference to activity");
};
subtest "Timeslot Association" => sub{
	my $module = new Module("IN2029", "Programming in C++");
	my $lecture = new Activity($module,"Lecture", "ALL");
	my $slot = new SimpleTimeslot("Tue", "Term 2", 12,00,2,00);
